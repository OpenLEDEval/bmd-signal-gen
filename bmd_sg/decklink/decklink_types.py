...
class DecklinkSDKProtocol(Protocol):
    ...
    def decklink_open_output_by_index(self, index: int) -> Optional[ctypes.c_void_p]:
    ...
    def decklink_set_pixel_format(self, handle: ctypes.c_void_p, format_index: int) -> int:
    ...
    def decklink_set_hdr_metadata(
        self, 
        handle: ctypes.c_void_p, 
        metadata: ctypes.POINTER(HDRMetadata)
    ) -> int:
