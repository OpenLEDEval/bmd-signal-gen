...
class ColorValidator:
    ...
    def validate_color(self, r: int, g: int, b: int, color_name: str = "Color") -> None:
        ...
        for component, name in [
            (r, f"{color_name} Red"),
            (g, f"{color_name} Green"),
            (b, f"{color_name} Blue"),
        ]:
            if not (self.min_val <= component <= self.max_val):
                raise ValueError(
                    f"{name} value {component} is out of range for {self.bit_depth}-bit format ({self.min_val}-{self.max_val})"
                )

    def validate_color_tuple(
        self, color_tuple: tuple[int, int, int], color_name: str = "Color"
    ) -> None:
...
class PatternGenerator:
    ...
    def _validate_roi(
        self, roi_x: int, roi_y: int, roi_width: int | None, roi_height: int | None
    ) -> tuple[int, int]:
        ...
        if (
            roi_x < 0
            or roi_y < 0
            or roi_x + roi_width > self.width
            or roi_y + roi_height > self.height
        ):
            raise ValueError(
                f"Region of interest ({roi_x},{roi_y},{roi_width},{roi_height}) is outside image boundaries ({self.width}x{self.height})"
            )
        return roi_width, roi_height

    def _draw_checkerboard_pattern(
        self,
        image: np.ndarray,
    ...
    def _print_pattern_info(
        self, pattern_type: PatternType, colors: list[tuple[int, int, int]]
    ):
        print(
    ...
    def _generate_solid(self, r: int, g: int, b: int) -> np.ndarray:
        ...
        self._draw_checkerboard_pattern(
            image, colors, self.roi_x, self.roi_y, self.roi_width, self.roi_height
        )
        return image

    def _generate_2color(
