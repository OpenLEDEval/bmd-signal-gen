...
DeckLinkSignalGen::DeckLinkSignalGen()
    : m_device(nullptr), m_output(nullptr), m_configuration(nullptr), m_frame(nullptr), m_displayMode(bmdModeHD1080p30), m_width(1920), m_height(1080), m_outputEnabled(false), m_pixelFormat(bmdFormat12BitRGBLE), m_formatsCached(false)
{
...
int DeckLinkSignalGen::createFrame()
{
    ...
    if (m_hdrMetadata.EOTF >= 0)
    {                       // Changed from m_eotfType to m_hdrMetadata.EOTF
...
extern "C"
{
    ...
    int decklink_get_supported_pixel_format_name(DeckLinkHandle handle, int index, char *name, int name_size)
    {
        ...
        signalGen->cacheSupportedFormats();
        if (index < 0 || index >= static_cast<int>(signalGen->getSupportedFormats().size()))
            return -1;
