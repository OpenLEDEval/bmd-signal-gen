#!/usr/bin/env python3
"""
Python wrapper for DeckLink color patch output using ctypes.
"""
import ctypes
import os
import sys
...
decklink.decklink_set_color.argtypes = [ctypes.c_void_p, ctypes.c_uint16, ctypes.c_uint16, ctypes.c_uint16]
decklink.decklink_set_color.restype = ctypes.c_int
...
class BMDDeckLink:
    ...
    def set_color(self, r, g, b):
        """Set the output color. Values must be in the correct range for the selected pixel format:
        - 8-bit:   0-255
        - 10-bit:  0-1023
        - 12-bit:  0-4095
        """
        ...
        res = decklink.decklink_set_color(self.handle, r, g, b)
        if res != 0:
            raise RuntimeError(f"Failed to set color (error {res})")
