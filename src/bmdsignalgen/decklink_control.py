from lib.bmd_decklink import BMDDeckLink, get_decklink_devices, PixelFormatType, EOTFType
...
def _initialize_decklink_device(device_index: int = 0, pixel_format_index: Optional[int] = None, 
                               show_logs: bool = True, use_global: bool = False):
    ...
    try:
        from lib.bmd_decklink import get_decklink_driver_version, get_decklink_sdk_version
        if show_logs:
        ...
        if device_index >= len(devices):
            return False, None, None, None, f"Device index {device_index} not found. Available devices: 0-{len(devices)-1}"
        ...
        if pixel_format_index is None:
            ...
            if pixel_format_index >= len(filtered_formats):
                return False, None, None, None, f"Pixel format index {pixel_format_index} not found"
        ...
        if use_global:
            print(f"DEBUG: Setting global variables - decklink_instance={decklink}, bit_depth={bit_depth}")
            decklink_instance = decklink
            decklink_bit_depth = bit_depth
            print(f"DEBUG: Global variables set - decklink_instance={decklink_instance}, decklink_bit_depth={decklink_bit_depth}")
...
def initialize_decklink_for_api(device_index: int = 0, pixel_format_index: Optional[int] = None):
...
def create_api_args(width: int, height: int, pattern: PatternType, colors: list, 
                   roi_x: int = 0, roi_y: int = 0, roi_width: Optional[int] = None, roi_height: Optional[int] = None,
                   duration: float = 5.0, no_hdr: bool = False, eotf: EOTFType = EOTFType.PQ,
                   max_cll: int = 1000, max_fall: int = 400):
    class ApiArgs:
        def __init__(self):
            self.width = width
            self.height = height
